...
kubeadm init --apiserver-advertise-address=$MASTER_IP --apiserver-cert-extra-sans=$MASTER_IP --kubernetes-version=${KUBE_VERSION} --ignore-preflight-errors=NumCPU --pod-network-cidr ${POD_CIDR} --node-name "$NODENAME"
